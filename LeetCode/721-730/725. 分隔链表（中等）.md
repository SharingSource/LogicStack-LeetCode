...
我们可以采取与 [(题解) 68. 文本左右对齐](https://mp.weixin.qq.com/s/NSEu2d7GGTqjXFKy3Aqrgg) 类似的思路（在 $68$ 中，填充空格的操作与本题一致：尽可能平均，无法均分时，应当使前面比后面多）。
...
public:
    vector<ListNode*> splitListToParts(ListNode* head, int k) {
        int cnt = 0;
        ListNode* tmp = head;
        while (tmp != nullptr && ++cnt > 0) tmp = tmp->next;
        int per = cnt / k;        
        vector<ListNode*> ans(k, nullptr);
        for (int i = 0, sumv = 1; i < k; i++, sumv++) {
            ans[i] = head;
            tmp = ans[i];
            int u = per;
            while (u-- > 1 && ++sumv > 0) tmp = tmp->next;
            int remain = k - i - 1;
            if (per != 0 && sumv + per * remain < cnt && ++sumv > 0) tmp = tmp->next;
            head = tmp != nullptr ? tmp->next : nullptr;

...
假设 $n$ 个硬币最多凑出 $x$ 行，根据等差数列求和公式 $S_n = n \times a_1 + \frac{n \times (n - 1)}{2} \times d$（首项和公差均为 $1$），可得 $n$ 和 $x$ 的关系：
...
$$
x <= \left \lfloor \frac{- 1 + \sqrt{1 + 8 \times n}}{2} \right \rfloor
$$
...
public:
    int arrangeCoins(int n) {
        return static_cast<int>((sqrt(1 + 8.0 * n) - 1) / 2);
    }
};
```
Python 代码：

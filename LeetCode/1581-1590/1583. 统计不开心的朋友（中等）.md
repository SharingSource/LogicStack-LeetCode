...
对每位朋友 `i`，`preferences[i]` 包含一份按亲近程度从高到低排列的朋友列表。
...
public:
    ...
    bool check(int x, int y, int u, int v) {
        return (map[x][u] > map[x][y] && map[u][x] > map[u][v]) ||
               (map[x][v] > map[x][y] && map[v][x] > map[v][u]);
    }
    int unhappyFriends(int n, vector<vector<int>>& preferences, vector<vector<int>>& pairs) {
        int m = pairs.size(), ans = 0;
        for (int i = 0; i < n; i++) {
...
class Solution:
        ...
        for i in range(m):
            ...
            for j in range(m):
                if i == j: 
                    continue
                u, v = pairs[j]
                if not xok and self.check(x, y, u, v, mapping):
                    xok = True
                if not yok and self.check(y, x, u, v, mapping):
                    yok = True
                if xok and yok: 
                    break
            if xok: 
    ...
    def check(self, x: int, y: int, u: int, v: int, mapping: list[list[int]]) -> bool:
        return (mapping[x][u] > mapping[x][y] and mapping[u][x] > mapping[u][v]) or \
               (mapping[x][v] > mapping[x][y] and mapping[v][x] > mapping[v][u])
```

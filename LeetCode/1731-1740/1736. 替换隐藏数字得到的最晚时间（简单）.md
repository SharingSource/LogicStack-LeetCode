...
class Solution:
    def maximumTime(self, time: str) -> str:
        res = []
        res.append('2' if time[0] == '?' and (time[1] == '?' or time[1] < '4') else '1' if time[0] == '?' else time[0])        
        res.append('3' if time[1] == '?' and res[0] == '2' else '9' if time[1] == '?' else time[1])
        res.append(':')
        res.append('5' if time[3] == '?' else time[3])
        res.append('9' if time[4] == '?' else time[4])
        return ''.join(res)
```
* 时间复杂度：$O(1)$
* 空间复杂度：$O(1)$

...
public:
    int longestSubsequence(vector<int>& arr, int d) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            f[i][1] = 1;
            int prev = arr[i] - d;
            if (hash[prev + M] != -1) f[i][1] = max(f[i][1], f[hash[prev + M]][1] + 1);
            hash[arr[i] + M] = i;
        }
...
class Solution:
    def longestSubsequence(self, arr: List[int], d: int) -> int:
        ...
        for i in range(1, n):
            ...
            prev = arr[i] - d
            if hmap[prev + M] != -1:
                f[i][1] = max(f[i][1], f[hmap[prev + M]][1] + 1)
            hmap[arr[i] + M] = i
        return max(f[n - 1][0], f[n - 1][1])
```
* 时间复杂度：令 $n$ 为数组长度，共有 $n \times 2$ 个状态需要被计算，每个状态转移的复杂度为 $O(1)$。整体复杂度为 $O(n)$
* 空间复杂度：$O(n)$
...
class Solution:
    def longestSubsequence(self, arr: List[int], d: int) -> int:
        ...
        hmap = [0] * N
        for i in arr:
            hmap[i + M] = hmap[i - d + M] + 1
            ans = max(ans, hmap[i + M])
        return ans
```

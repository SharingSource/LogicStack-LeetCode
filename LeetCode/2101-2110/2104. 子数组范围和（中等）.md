...
`nums` 中，子数组的范围是子数组中最大元素和最小元素的差值。
...
public:
    long long subArrayRanges(vector<int>& nums) {
        ...
        for (int len = 2; len <= n; len++) {
            for (int l = 0; l + len - 1 < n; l++) {
                int r = l + len - 1;
                minVal[l][r] = min(nums[r], minVal[l][r - 1]);
                maxVal[l][r] = max(nums[r], maxVal[l][r - 1]);
            }
        }
        long long ans = 0;
        for (int i = 0; i < n; i++) {
    ...
    def subArrayRanges(self, nums: List[int]) -> int:
        ...
        for length in range(2, n + 1):
            for l in range(n - length + 1):
                r = l + length - 1
                min_val[l][r] = min(nums[r], min_val[l][r - 1])
                max_val[l][r] = max(nums[r], max_val[l][r - 1])
        ans = 0
        for i in range(n):
            for j in range(i + 1, n):
                ans += (max_val[i][j] - min_val[i][j])
        return ans
```
...
public:
    long long subArrayRanges(vector<int>& nums) {
        int n = nums.size();
        vector<long long> minCnt = getCnt(nums, true);
        vector<long long> maxCnt = getCnt(nums, false);
        long long ans = 0;
        for (int i = 0; i < n; i++) ans += (maxCnt[i] - minCnt[i]) * nums[i];
        return ans;
    }
    vector<long long> getCnt(vector<int>& nums, bool isMin) {
        ...
        deque<int> dq;
        for (int i = 0; i < n; i++) {
            while (!dq.empty() && (isMin ? nums[dq.back()] >= nums[i] : nums[dq.back()] <= nums[i])) dq.pop_back();
            a[i] = dq.empty() ? -1 : dq.back();
            dq.push_back(i);
        }
        dq.clear();
        for (int i = n - 1; i >= 0; i--) {
            while (!dq.empty() && (isMin ? nums[dq.back()] > nums[i] : nums[dq.back()] < nums[i])) dq.pop_back();
            b[i] = dq.empty() ? n : dq.back();
            dq.push_back(i);
        }
        vector<long long> ans(n);
        for (int i = 0; i < n; i++) ans[i] = (i - a[i]) * 1LL * (b[i] - i);
        return ans;
    }
};
```
...
class Solution:
    def subArrayRanges(self, nums: List[int]) -> int:
        n = len(nums)
        min_cnt = self.get_cnt(nums, True)
        max_cnt = self.get_cnt(nums, False)
        ans = 0
        for i in range(n):
            ans += (max_cnt[i] - min_cnt[i]) * nums[i]
        return ans
    
    def get_cnt(self, nums: List[int], is_min: bool) -> List[int]:
        n = len(nums)
        a, b = [0] * n, [0] * n
        dq = deque()
        for i in range(n):
            while dq and (nums[dq[-1]] >= nums[i] if is_min else nums[dq[-1]] <= nums[i]):
                dq.pop()
            a[i] = -1 if not dq else dq[-1]
            dq.append(i)
        dq.clear()
        for i in range(n - 1, -1, -1):
            while dq and (nums[dq[-1]] > nums[i] if is_min else nums[dq[-1]] < nums[i]):
                dq.pop()
            b[i] = n if not dq else dq[-1]
            dq.append(i)
        ans = [0] * n
        for i in range(n):
            ans[i] = (i - a[i]) * (b[i] - i)

...
public:
    int maxRotateFunction(vector<int>& nums) {
        int n = nums.size();
        vector<int> sumv(2 * n + 10, 0);
        for (int i = 1; i <= 2 * n; i++) sumv[i] = sumv[i - 1] + nums[(i - 1) % n];
        int ans = 0;
        for (int i = 1; i <= n; i++) ans += nums[i - 1] * (i - 1);
        for (int i = n + 1, cur = ans; i < 2 * n; i++) {
            cur += nums[(i - 1) % n] * (n - 1);
            cur -= sumv[i - 1] - sumv[i - n];
            if (cur > ans) ans = cur;
        }
        return ans;
    }
...
class Solution:
    def maxRotateFunction(self, nums: List[int]) -> int:
        ...
        for i in range(1, 2 * n + 1):
            sum_arr[i] = sum_arr[i - 1] + nums[(i - 1) % n]
        ans = 0
...
function maxRotateFunction(nums: number[]): number {
    const n = nums.length;
    const sumv = new Array(2 * n + 10).fill(0);
    for (let i = 1; i <= 2 * n; i++) sumv[i] = sumv[i - 1] + nums[(i - 1) % n];
    let ans = 0;
    for (let i = 1; i <= n; i++) ans += nums[i - 1] * (i - 1);
    for (let i = n + 1, cur = ans; i < 2 * n; i++) {
        cur += nums[(i - 1) % n] * (n - 1);
        cur -= sumv[i - 1] - sumv[i - n];
        if (cur > ans) ans = cur;
    }

...
public:
    int maxChunksToSorted(vector<int>& arr) {
        ...
        for (int i = 0; i < n; i++) {
            int a = arr[i], b = clone[i];
            if (cntMap.find(a) != cntMap.end() && cntMap[a] == -1) tot--;
            else if (cntMap.find(a) == cntMap.end() || cntMap[a] == 0) tot++;
            cntMap[a] = (cntMap.find(a) != cntMap.end() ? cntMap[a] : 0) + 1;
            if (cntMap.find(b) != cntMap.end() && cntMap[b] == 1) tot--;
            else if (cntMap.find(b) == cntMap.end() || cntMap[b] == 0) tot++;
            cntMap[b] = (cntMap.find(b) != cntMap.end() ? cntMap[b] : 0) - 1;            
            if (tot == 0) ans++;
        }
        return ans;
    }

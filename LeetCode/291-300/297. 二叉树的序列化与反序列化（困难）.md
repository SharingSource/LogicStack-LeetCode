...
根据节点值的数据范围 `-1000 <= Node.val <= 1000`，我们可以建立占位节点 `emptyNode` 用来代指空节点（`emptyNode.val = INF`）。
...
public:
    ...
    string serialize(TreeNode* root) {
        ...
        while (!q.empty()) {
            ...
            if (node->val != INF) {
                q.push(node->left ? node->left : new TreeNode(INF));
                q.push(node->right ? node->right : new TreeNode(INF));
            }
        }
        return res;
    }
    TreeNode* deserialize(string data) {
        ...
        for (int i = 1; i < nums.size() - 1; i += 2) {
            ...
            if (a != INF) {
                parent->left = new TreeNode(a);
                q.push(parent->left);
            }
            if (b != INF) {
                parent->right = new TreeNode(b);
                q.push(parent->right);
            }
        }
        return root;

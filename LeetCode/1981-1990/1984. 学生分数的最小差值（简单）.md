...
从数组中选出任意 $k$ 名学生的分数，使这 $k$ 个分数间「最高分」和「最低分」的差值达到最小化。
...
class Solution {
    public int minimumDifference(int[] nums, int k) {
        Arrays.sort(nums);
        int n = nums.length, ans = nums[k - 1] - nums[0];
        for (int i = k; i < n; i++) ans = Math.min(ans, nums[i] - nums[i - k + 1]);
        return ans;
    }
}
```
...
public:
    int minimumDifference(vector<int>& nums, int k) {
        ...
        for (int i = k; i < n; i++) ans = min(ans, nums[i] - nums[i - k + 1]);
        return ans;
    }
};
```
...
function minimumDifference(nums: number[], k: number): number {
    nums.sort((a, b) => a - b);
    let n = nums.length, ans = nums[k - 1] - nums[0];
    for (let i = k; i < n; i++) ans = Math.min(ans, nums[i] - nums[i - k + 1]);
    return ans;
};
```
...
这是我们「刷穿 LeetCode」系列文章的第 `No.1984` 篇，系列开始于 2021/01/01，截止于起始日 LeetCode 上共有 1916 道题目，部分是有锁题，我们将先把所有不带锁的题目刷完。

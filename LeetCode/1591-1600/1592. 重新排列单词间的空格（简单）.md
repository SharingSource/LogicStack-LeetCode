...
如果不能重新平均分配所有空格，请将多余的空格放置在字符串末尾，这也意味着返回的字符串应当与原 `text` 字符串的长度相等。
...
public:
    string reorderSpaces(string text) {
        ...
        int m = words.size();
        int gap = (m <= 1) ? 0 : cnt / (m - 1);
        int extra = (m <= 1) ? cnt : cnt % (m - 1);        
        for (int i = 0; i < m; ++i) {
            result += words[i];
            if (i < m - 1) result += string(gap, ' ');
        }
        result += string(extra, ' ');
        return result;
    }
...
class Solution:
    def reorderSpaces(self, text: str) -> str:
        ...
        words = text.split()
        m = len(words)
        gap = cnt // (m - 1) if m > 1 else 0
        extra = cnt - gap * (m - 1) if m > 1 else cnt

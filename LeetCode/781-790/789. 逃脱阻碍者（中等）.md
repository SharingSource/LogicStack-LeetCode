...
每一回合，你和阻碍者们可以同时向东，西，南，北四个方向移动，每次可以移动到距离原位置 1 个单位的新位置。当然，也可以选择不动。所有动作同时发生。
...
class Solution:
    ...
    def escapeGhosts(self, gs: List[List[int]], t: List[int]) -> bool:
        cur = self.dist(0, 0, t[0], t[1])
        for g in gs:
            if self.dist(g[0], g[1], t[0], t[1]) <= cur:
                return False
        return True
```

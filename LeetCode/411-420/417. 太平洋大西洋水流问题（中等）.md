...
public:
    ...
    void bfs(queue<pair<int, int>>& d, vector<vector<bool>>& res) {
        while (!d.empty()) {
            auto [x, y] = d.front();
            d.pop();
            int t = g[x][y];
...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        d1, d2 = deque(), deque()
        res1 = [[False] * self.n for _ in range(self.m)]
        res2 = [[False] * self.n for _ in range(self.m)]
    ...
    def bfs(self, d: deque, res: List[List[bool]]):
        ...
        while d:
            ...
            for dx, dy in dirs:
                ...
                if nx < 0 or nx >= self.m or ny < 0 or ny >= self.n:
                    continue
                if res[nx][ny] or self.g[nx][ny] < t:
                    continue
                d.append((nx, ny))
                res[nx][ny] = True
```
...
public:
    ...
    void dfs(int x, int y, vector<vector<bool>>& res) {
        res[x][y] = true;
        for (auto& di : dirs) {
            int nx = x + di[0];
            int ny = y + di[1];
...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        self.m = len(heights)
        self.n = len(heights[0])
        self.res1 = [[False] * self.n for _ in range(self.m)]
        self.res2 = [[False] * self.n for _ in range(self.m)]
        for i in range(self.m):
            for j in range(self.n):
                if i == 0 or j == 0:
                    if not self.res1[i][j]:
                        self.dfs(i, j, self.res1)
                if i == self.m - 1 or j == self.n - 1:
                    if not self.res2[i][j]:
                        self.dfs(i, j, self.res2)
        ans = []
        for i in range(self.m):
            for j in range(self.n):
                if self.res1[i][j] and self.res2[i][j]:
                    ans.append([i, j])
        return ans
    
    def dfs(self, x: int, y: int, res: List[List[bool]]):
        res[x][y] = True
        dirs = [(1,0), (-1,0), (0,1), (0,-1)]
        for dx, dy in dirs:
            nx, ny = x + dx, y + dy
            if nx < 0 or nx >= self.m or ny < 0 or ny >= self.n:
                continue
            if res[nx][ny] or self.g[nx][ny] < self.g[x][y]:
                continue
            self.dfs(nx, ny, res)
```
...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        for i in range(self.m):
            for j in range(self.n):
                ...
                if i == 0 or j == 0:
                    if not self.query(self.p1, self.S, idx):
                        self.dfs(self.p1, self.S, i, j)
                if i == self.m - 1 or j == self.n - 1:
                    if not self.query(self.p2, self.T, idx):
                        self.dfs(self.p2, self.T, i, j)
        ans = []
        for i in range(self.m):
            for j in range(self.n):
                idx = self.get_idx(i, j)
                if (self.query(self.p1, self.S, idx) and 
                    self.query(self.p2, self.T, idx)):
                    ans.append([i, j])
        return ans
    
    def union(self, p, a, b):
        p[self.find(p, a)] = p[self.find(p, b)]
    ...
    def query(self, p, a, b):
        return self.find(p, a) == self.find(p, b)
    
    def get_idx(self, x, y):
        return x * self.n + y
    
    def dfs(self, p, ori, x, y):
        self.union(p, ori, self.get_idx(x, y))
        for dx, dy in self.dirs:
            nx, ny = x + dx, y + dy
            if nx < 0 or nx >= self.m or ny < 0 or ny >= self.n:
                continue
            if (self.query(p, ori, self.get_idx(nx, ny)) or 
                self.g[nx][ny] < self.g[x][y]):
                continue
            self.dfs(p, ori, nx, ny)
```
* 时间复杂度：$O(n \times m)$
* 空间复杂度：$O(n \times m)$

...
class Solution:
    ...
    def dfs(self, u: int) -> None:
        if u == self.n - 1:
            ...
            return
        for next_node in self.g[u]:
            self.cur.append(next_node)
            self.dfs(next_node)
            self.cur.pop()
```
TypeScript 代码：
...
```
* 时间复杂度：共有 $n$ 个节点，每个节点有选和不选两种决策，总的方案数最多为 $2^n$，对于每个方案最坏情况需要 $O(n)$ 的复杂度进行拷贝并添加到结果集。整体复杂度为 $O(n \times 2^n)$
* 空间复杂度：最多有 $2^n$ 种方案，每个方案最多有 $n$ 个元素。整体复杂度为 $O(n \times 2^n)$

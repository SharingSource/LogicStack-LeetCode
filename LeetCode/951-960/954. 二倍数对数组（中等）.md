...
public:
    ...
    bool canReorderDoubled(vector<int>& arr) {
        ...
        vector<int> list;
        for (int i : arr) {
            if (++cnts[i + M] == 1) list.push_back(i);
        }
...
class Solution:
    def canReorderDoubled(self, arr: List[int]) -> bool:
        ...
        unique_list.sort(key=lambda x: abs(x))
        for num in unique_list:
            if cnts[num * 2 + M] < cnts[num + M]:
                return False
            cnts[num * 2 + M] -= cnts[num + M]
        return True
```
...
public:
    ...
    bool canReorderDoubled(vector<int>& arr) {
        ...
        vector<int> list;
        for (int i : arr) {
            if (++cnts[i + M] == 1 && i != 0) list.push_back(i);
        }
        if (cnts[M] % 2 != 0) return false;
        deque<int> d;
        for (int i : list) {
        ...
        while (!d.empty()) {
            ...
            cnts[t * 2 + M] -= cnts[t + M];
            in[t * 2 + M] -= cnts[t + M];
            if (in[t * 2 + M] == 0 && cnts[t * 2 + M] != 0) d.push_back(t * 2);
            in[t * 4 + M] -= cnts[t + M];
            if (in[t * 4 + M] == 0 && cnts[t * 4 + M] != 0) d.push_back(t * 4);
        }
        return true;
    }
};
...
class Solution:
    def canReorderDoubled(self, arr: List[int]) -> bool:
        ...
        for num in unique_list:
            if num % 2 == 0:
                in_degree[num + M] = cnts[num // 2 + M]
                if in_degree[num + M] == 0:
                    dq.append(num)
            else:
                dq.append(num)
        while dq:
            t = dq.popleft()
            if cnts[t * 2 + M] < cnts[t + M]:
                return False
            cnts[t * 2 + M] -= cnts[t + M]
            in_degree[t * 2 + M] -= cnts[t + M]
            if in_degree[t * 2 + M] == 0 and cnts[t * 2 + M] != 0:
                dq.append(t * 2)
            in_degree[t * 4 + M] -= cnts[t + M]
            if in_degree[t * 4 + M] == 0 and cnts[t * 4 + M] != 0:
                dq.append(t * 4)
        return True
```

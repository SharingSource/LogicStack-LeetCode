...
public:
    vector<int> platesBetweenCandles(string s, vector<vector<int>>& qs) {
        ...
        vector<int> sumv(s.length() + 1, 0);
        vector<int> list;
        for (int i = 0; i < s.length(); i++) {
            if (s[i] == '|') list.push_back(i);
            sumv[i + 1] = sumv[i] + (s[i] == '*' ? 1 : 0);
        }
...
class Solution:
    def platesBetweenCandles(self, s: str, qs: List[List[int]]) -> List[int]:
        ...
        for i in range(n):
            if s[i] == '|':
                list_candles.append(i)
            sumv[i + 1] = sumv[i] + (1 if s[i] == '*' else 0)
        if not list_candles:
            return ans
        for i in range(m):
            a, b = qs[i]
            c, d = -1, -1
            # 找到 a 右边最近的蜡烛
            l, r = 0, len(list_candles) - 1
            while l < r:
                mid = l + r >> 1
                if list_candles[mid] >= a:
                    r = mid
                else:
                    l = mid + 1
            if list_candles[r] >= a:
...
public:
    vector<int> platesBetweenCandles(string s, vector<vector<int>>& qs) {
        ...
        for (int i = 0; i < qs.size(); i++) {
            int a = qs[i][0], b = qs[i][1];
            int c = r[a], d = l[b];
            if (c != -1 && c <= d) ans[i] = sumv[d + 1] - sumv[c];
        }
        return ans;
    }
};
...
function platesBetweenCandles(s: string, qs: number[][]): number[] {
    const n = s.length, m = qs.length;
    const l = new Array(n).fill(-1), r = new Array(n).fill(-1);
    const sum = new Array(n + 1).fill(0);
    for (let i = 0, j = n - 1, p = -1, q = -1; i < n; i++, j--) {
        if (s[i] === '|') p = i;
        if (s[j] === '|') q = j;
        l[i] = p; r[j] = q;
    ...
    for (let i = 0; i < m; i++) {
        const [a, b] = qs[i];
        const c = r[a], d = l[b];
        if (c !== -1 && c <= d) ans.push(sum[d + 1] - sum[c]);
        else ans.push(0);
    }
    return ans;
};

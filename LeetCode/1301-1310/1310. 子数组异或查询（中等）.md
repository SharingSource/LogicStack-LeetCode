...
public:
    ...
    vector<int> xorQueries(vector<int>& arr, vector<vector<int>>& qs) {
        n = arr.size();
        for (int i = 1; i <= n; i++) add(i, arr[i - 1]);
...
class Solution:
    ...
    def xorQueries(self, arr, qs):
        self.n = len(arr)
        for i in range(1, self.n + 1):
            self.add(i, arr[i - 1])
        ans = []
        for l, r in qs:
            ans.append(self.query(r + 1) ^ self.query(l))
        return ans
```
...
```Python
class Solution:
    def xorQueries(self, arr: List[int], qs: List[List[int]]) -> List[int]:
        n, m = len(arr), len(qs)
        sumv = [0] * (n + 1)
        for i in range(1, n + 1):
            sumv[i] = sumv[i - 1] ^ arr[i - 1]
        ans = []
        for i in range(m):
            l, r = qs[i][0] + 1, qs[i][1] + 1
            ans.append(sumv[r] ^ sumv[l - 1])
        return ans
```

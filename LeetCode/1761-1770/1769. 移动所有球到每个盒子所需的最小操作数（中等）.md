...
这是 LeetCode 上的 **[1769. 移动所有球到每个盒子所需的最小操作数](https://mp.weixin.qq.com/s/3u8Ln2ZBPHonHFH0xxqbKQ)** ，难度为 **中等**。
...
返回一个长度为 `n` 的数组 `answer`，其中 `answer[i]` 是将所有小球移动到第 `i` 个盒子所需的最小 操作数。
...
public:
    vector<int> minOperations(string boxes) {
        ...
        for (int i = 0, cur = 0, step = 0; i < n; i++) {
            step += cur; cur += boxes[i] - '0'; l[i] = step;
        }
        for (int i = n - 1, cur = 0, step = 0; i >= 0; i--) {
            step += cur; cur += boxes[i] - '0'; r[i] = step;
        }
        vector<int> ans(n);
        for (int i = 0; i < n; i++) ans[i] = l[i] + r[i];
        return ans;

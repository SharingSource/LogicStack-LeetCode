        ...
        for (int i = 0; i < n; i++) {
            if (s[i] == '?') {
                for (int c = 0; c < 3 && s[i] == '?'; c++) {
                    bool ok = true;
                    if (i > 0 && s[i - 1] == ('a' + c)) ok = false;
                    if (i < n - 1 && s[i + 1] == ('a' + c)) ok = false;
                    if (ok) s[i] = 'a' + c;
                }
            }
        }
...
class Solution:
    def modifyString(self, s: str) -> str:
        ...
        for i in range(n):
            if lst[i] == '?':
                for c in range(3):
                    cur = chr(ord('a') + c)
                    ok = True
                    if i > 0 and lst[i - 1] == cur:
                        ok = False
                    if i < n - 1 and lst[i + 1] == cur:
                        ok = False
                    if ok:
                        lst[i] = cur
                        break
        return ''.join(lst)
```
* 时间复杂度：会处理 `s` 中的所有字符，若当前字符为 `?` 时，会检查与前后字符的关系，最多在 $3$ 个字符内找到可替换的字符，复杂度为 $O(n \times C)$，其中 $C = 3$ 
* 空间复杂度：$O(n)$

        ...
        for (int len = 1; len <= n; len++) {
            for (int l = 0; l + len - 1 < n; l++) {
                ...
                if (len == 1) {
                    ...
                    f[l][r] = max(f[l][r], f[l + 1][r - 1] + (s[l] == s[r] ? 2 : 0));
                }
            }
        }
        return f[0][n - 1];
...
class Solution:
    def longestPalindromeSubseq(self, s: str) -> int:
        ...
        for lenv in range(1, n+1):
            for l in range(n - lenv + 1):
                r = l + lenv - 1
                if lenv == 1:
                    f[l][r] = 1
                elif lenv == 2:
                    f[l][r] = 2 if s[l] == s[r] else 1
                else:
                    f[l][r] = max(f[l + 1][r], f[l][r - 1])
                    f[l][r] = max(f[l][r], f[l + 1][r - 1] + (2 if s[l] == s[r] else 0))
        return f[0][n - 1]
```
TypeScript 代码：
```TypeScript
    ...
    for (let len = 1; len <= n; len++) {
        for (let l = 0; l + len - 1 < n; l++) {
            ...
            if (len === 1) {
                ...
                f[l][r] = s.charAt(l) === s.charAt(r) ? 2 : 1;
            } else {
                f[l][r] = Math.max(f[l + 1][r], f[l][r - 1]);
                f[l][r] = Math.max(f[l][r], f[l + 1][r - 1] + (s.charAt(l) === s.charAt(r) ? 2 : 0));
            }
        }
    }
    return f[0][n - 1];

...
public:
    int countValidWords(string sentence) {
        ...
        for (int i = 0; i < n; ) {
            ...
            if (check(sentence.substr(i, j - i))) ans++;
            i = j + 1;
        }
        return ans;
    }
...
function check(s: string): boolean {
    const n = s.length;
    if (n === 0) return false;
    for (let i = 0, c1 = 0, c2 = 0; i < n; i++) {
        const c = s[i];
        if (/\d/.test(c)) return false;
        if (c === ' ') return false;
        if (c === '-' && ++c1 >= 0) {
            if (c1 > 1 || i === 0 || i === n-1) return false;
            if (!/[a-z]/.test(s[i-1]) || !/[a-z]/.test(s[i+1])) return false;
        }
        if ('!.,'.includes(c) && ++c2 >= 0) {
            if (c2 > 1 || i !== n-1) return false;
        }

...
给你一个 下标从 `0` 开始 的整数数组 `nums`，返回满足下述条件的不同四元组 `(a, b, c, d)` 的数目：`nums[a]+nums[b]+nums[c]=nums[d]`，且 `a < b < c < d`。
    ...
    int countQuadruplets(vector<int>& nums) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            for (int j = 0; j < 110; j++) {
                for (int k = 0; k < 4; k++) {
                    f[i][j][k] += f[i - 1][j][k];
                    if (j >= t && k >= 1) f[i][j][k] += f[i - 1][j - t][k - 1];
                }
            }
        }
        for (int i = 3; i < n; i++) ans += f[i][nums[i]][3];
...
更进一步，利用 $f[i][j][k]$ 仅依赖于 $f[i - 1][j][k]$ 和 `j` `k` 维度值更小的 $f[i - 1][X][X]$，可进行维度优化，并在转移过程中统计答案。
...
public:
    int countQuadruplets(vector<int>& nums) {
        int n = nums.size(), ans = 0;
        vector<vector<int>> f(110, vector<int>(4, 0));
        f[0][0] = 1;
        for (int i = 1; i <= n; i++) {
            int t = nums[i - 1];
            ans += f[t][3];

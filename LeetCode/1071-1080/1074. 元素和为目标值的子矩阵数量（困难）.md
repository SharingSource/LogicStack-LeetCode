...
数据范围是 $10^2$，对应的计算量是 $10^8$，处于超时边缘，但当我们枚举「矩阵右下角」$(i,j)$ 的时候，我们只需要搜索位于 $(i,j)$ 的左上方的点 $(p,q)$ 作为「矩阵左上角」，所以其实我们是取不满 $m^2 \times n^2$ 的，但仍然具有超时风险（2024/11/27 Java 测试可通过）。
...
**这个分析思路在 [（题解）363. 矩形区域不超过 K 的最大数值和](https://mp.weixin.qq.com/s/aiOEmqb8j9iCIssUYSV-Iw) 详细讲过，没有印象的同学可以翻翻。这道题一定程度上是那道题的简化版，在本题我们只需要找到矩阵和为 $target$ 的值，因此只需要使用「哈希表」来记录出现过的值即可，而在 [（原题）363. 矩形区域不超过 K 的最大数值和](https://mp.weixin.qq.com/s/aiOEmqb8j9iCIssUYSV-Iw) 中，我们需要找到和不超过 $k$ 的最大子矩阵，因此还涉及「有序集合 + 二分」。**
...
![](https://pic.leetcode-cn.com/1622250565-sdYzhq-image.png)
...
public:
    int numSubmatrixSumTarget(vector<vector<int>>& mat, int t) {
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= m; j++) {
                sumv[i][j] = sumv[i - 1][j] + sumv[i][j - 1] - sumv[i - 1][j - 1] + mat[i - 1][j - 1];
            }
        }
        for (int top = 1; top <= n; top++) {
            for (int bot = top; bot <= n; bot++) {
                ...
                for (int r = 1; r <= m; r++) {
                    cur = sumv[bot][r] - sumv[top - 1][r];
                    if (cur == t) ans++;
                    if (map.count(cur - t)) ans += map[cur - t];
                    map[cur] = map[cur] + 1;
                }
            }
        }
...
```Python
class Solution:
    def numSubmatrixSumTarget(self, mat: List[List[int]], t: int) -> int:
        n, m, ans = len(mat), len(mat[0]), 0
        sumv = [[0] * (m + 1) for _ in range(n + 1)]
...
TypeScript 代码：
```TypeScript
function numSubmatrixSumTarget(mat: number[][], t: number): number {
    let n = mat.length, m = mat[0].length, ans = 0;
    const sum = [[0].concat(...new Array(m + 1)).map(() => 0)];
    for (let i = 1; i <= n; i++) {
        sum[i] = [0];
        for (let j = 1; j <= m; j++) {
            sum[i][j] = sum[i - 1][j] + sum[i][j - 1] - sum[i - 1][j - 1] + mat[i - 1][j - 1];
    ...
    for (let top = 1; top <= n; top++) {
        for (let bot = top; bot <= n; bot++) {
            let cur = 0;
            const map: { [key: number]: number } = {};
            for (let r = 1; r <= m; r++) {
                cur = sum[bot][r] - sum[top - 1][r];
                if (cur === t) ans++;
                if (cur - t in map) ans += map[cur - t];
                map[cur] = (map[cur] || 0) + 1;
            }
        }
    }
...
1. 【时间复杂度】在上述解法中，我们采用的是**固定上下边界，移动右边界，通过哈希表找左边界**的做法，复杂度为 $O(m \times n^2)$；自然也能改为**固定左右边界，移动下边界，通过哈希表找上边界**做法，复杂度为 $O(m^2 \times n)$。那么我们要如何调整代码，才能最大化「哈希表」带来的优化效果呢？此时的复杂度又是多少？

2. 【空间复杂度】我们空间复杂度的瓶颈在「二维前缀和」上，但注意到无论我们采取「固定上下边界」还是「固定左右边界」的做法，扫描原矩阵的方向都是固定的，那么是否可以不预处理「二维前缀和」呢？从而将空间复杂度降低到 $O(\max(n, m))$ 呢？

上述两问，你都可以在 [（题解）363. 矩形区域不超过 K 的最大数值和](https://mp.weixin.qq.com/s/aiOEmqb8j9iCIssUYSV-Iw) 中找到答案。

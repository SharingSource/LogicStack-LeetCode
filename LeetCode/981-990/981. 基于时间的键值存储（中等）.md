...
public:
    struct Node {
        string k, v;
        int t;
        Node(string _k, string _v, int _t) : k(_k), v(_v), t(_t) {}
    };

    unordered_map<string, vector<Node>> map;

    void set(string k, string v, int t) {
        map[k].emplace_back(k, v, t);
    }
    ...
    string get(string k, int t) {
        ...
        auto iter = upper_bound(nodes.begin(), nodes.end(), t,
            [](int timestamp, const Node& node) {
                return timestamp < node.t;
            });
        return iter != nodes.begin() ? (--iter)->v : "";
    }
};
```

...
现在给定另一个字符串 `p` 。返回 `s` 中 唯一 的 `p` 的非空子串的数量 。 
...
public:
    int findSubstringInWraproundString(string p) {
        ...
        vector<int> maxv(26, 0);
        if (n == 0) return 0;
        maxv[p[0] - 'a'] = 1;
        for (int i = 1, j = 1; i < n; i++) {
            int c = p[i] - 'a', pc = p[i - 1] - 'a';
            if ((pc == 25 && c == 0) || (pc + 1 == c)) j++;
            else j = 1;
            maxv[c] = max(maxv[c], j);
        }
        for (int i = 0; i < 26; i++) ans += maxv[i];
        return ans;
    }
};
```
...
class Solution:
    def findSubstringInWraproundString(self, p: str) -> int:
        ...
        if n == 0:
            return 0
        maxv[ord(p[0]) - ord('a')] = 1
        j = 1
        for i in range(1, n):
            c = ord(p[i]) - ord('a')
            p_char = ord(p[i - 1]) - ord('a')
            if (p_char == 25 and c == 0) or (p_char + 1 == c):
                j += 1
            else:
                j = 1 
            maxv[c] = max(maxv[c], j)

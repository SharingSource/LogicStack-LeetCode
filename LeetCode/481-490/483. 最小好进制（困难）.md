...
$$
n = k^0 + k^1 + k^2 + ... + k^s < C_{s}^{0} \times k^0 + C_{s}^{1} \times k^1 + ... + C_{s}^{s} \times k^s = (k + 1)^{s}
$$
...
public:
    string smallestGoodBase(string n) {
        long long m = stoll(n);
        int maxv = log2(m) + 1;
        for (int len = maxv; len >= 3; len--) {
            long long k = pow(m, 1.0 / (len - 1));
            long long res = 0;
            for (int i = 0; i < len; i++) res = res * k + 1;
...
class Solution:
    def smallestGoodBase(self, n: str) -> str:
        m = int(n)
        maxv = int(math.log(m, 2) + 1)
        for lenv in range(maxv, 2, -1):
            k = int(m ** (1 / (lenv - 1)))
            res = 0
            for _ in range(lenv):
                res = res * k + 1 

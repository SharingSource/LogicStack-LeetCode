...
class Solution:
    def findBall(self, grid):
        ...
        return [self.getVal(i) for i in range(self.n)]

    def getVal(self, x):
        r, c = 0, x
        while r < self.m:
            ne = c + self.g[r][c]
            if ne < 0 or ne >= self.n:
                return -1
            if self.g[r][c] != self.g[r][ne]:
                return -1
            r, c = r + 1, ne
        return c
```
...
```TypeScript
let n, m, g;
function getVal(x: number): number {
    let r = 0, c = x;
    while (r < m) {
        const ne = c + g[r][c];
        if (ne < 0 || ne >= n) return -1;
...
function findBall(grid: number[][]): number[] {
    ...
    for (let i = 0; i < n; i++) ans[i] = getVal(i);
    return ans;
};
```
* 时间复杂度：$O(m \times n)$

...
苹果数量很大，但产生苹果的天数最多为 $2 \times 10^4$，因此我们以二元组 `(最后食用日期, 当日产生苹果数量)` 的形式存入「小根堆」进行维护。
...
* $a_1$ 为 $(b_2, b_3, ..., b_m)$ 中的某个，假设为 $b_i = a_1$，由于 $ \frac{b_{i}}{a_{1}}$ 在贪心解中的先出堆（过期时间最早），因此必然也有 $ \frac{b_{i}}{a_{1}}$ 过期时间早于等于 $b_1$ 的过期时间，此时将 $b_1$ 放到 $b_i$ 的位置仍是合法（过期时间比 $ \frac{b_{i}}{a_{1}}$ 要晚），即将 $b_1$ 和 $ \frac{b_{i}}{a_{1}}$ 交换位置，最优序列合法性仍有保证，同时长度不变，结果不会变差。
...
class Solution:
    def eatenApples(self, apples: List[int], days: List[int]) -> int:
        heap = []
        n, ans, t = len(apples), 0, 0
        while t < n or heap:
            if t < n and apples[t] > 0:
                heapq.heappush(heap, (t + days[t] - 1, apples[t]))
            while heap and heap[0][0] < t:
                heapq.heappop(heap)
            if heap:
                expire, num = heapq.heappop(heap)
                ans += 1
                num -= 1
                if num > 0 and expire > t:
                    heapq.heappush(heap, (expire, num))
            t += 1
        return ans
```

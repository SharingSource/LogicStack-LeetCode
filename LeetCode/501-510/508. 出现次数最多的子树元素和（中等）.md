...
public:
    ...
    vector<int> findFrequentTreeSum(TreeNode* root) {
        ...
        for (const auto& kv : map) {
            if (kv.second == maxv) ans.push_back(kv.first);
        }
        return ans;
    }
    int dfs(TreeNode* root) {
...
class Solution:
    def findFrequentTreeSum(self, root: Optional[TreeNode]) -> List[int]:
        ...
        self.maxv = 0
        self.dfs(root)
        return [k for k, v in self.mapping.items() if v == self.maxv]

    def dfs(self, root: TreeNode) -> int:
        if not root:
            return 0
        cur = root.val + self.dfs(root.left) + self.dfs(root.right)
        self.mapping[cur] = self.mapping.get(cur, 0) + 1
        self.maxv = max(self.maxv, self.mapping[cur])
        return cur
```
TypeScript 代码：
```TypeScript
...
function dfs(root: TreeNode | null): number {
    ...
    maxv = Math.max(maxv, map.get(cur) || 0);
    return cur;
}

...
class Solution {
public:
    vector<bool> canEat(vector<int>& cs, vector<vector<int>>& qs) {
        int n = qs.size();
        int m = cs.size();
        vector<bool> ans(n, false);
        vector<long long> sumv(m + 1, 0);
        for (int i = 1; i <= m; i++) sumv[i] = sumv[i - 1] + cs[i - 1];
        for (int i = 0; i < n; i++) {
            int t = qs[i][0], d = qs[i][1] + 1, c = qs[i][2];
            long long a = sumv[t] / c + 1;
            long long b = sumv[t + 1];
            ans[i] = (a <= d) && (d <= b);
        }        
        return ans;
    }
...
```Python
class Solution:
    def canEat(self, cs: List[int], qs: List[List[int]]) -> List[bool]:
        n, m = len(qs), len(cs)
        ans = [False] * n
        sumv = [0] * (m + 1)
        for i in range(1, m + 1):
            sumv[i] = sumv[i - 1] + cs[i - 1]
        for i in range(n):
            t, d, c = qs[i][0], qs[i][1] + 1, qs[i][2]
            a, b = sumv[t] // c + 1, sumv[t + 1]
            ans[i] = a <= d <= b
        return ans
```

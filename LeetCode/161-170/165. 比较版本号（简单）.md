...
public:
    ...
    int compareVersion(string version1, string version2) {
        vector<string> ss1 = split(version1);
        vector<string> ss2 = split(version2);
...
class Solution:
    def compareVersion(self, version1: str, version2: str) -> int:
        ss1 = list(map(int, version1.split('.')))
        ss2 = list(map(int, version2.split('.')))
        n, m = len(ss1), len(ss2)
        i, j = 0, 0
        while i < n or j < m:
            a = ss1[i] if i < n else 0
            b = ss2[j] if j < m else 0
            if a != b:
...
function compareVersion(version1: string, version2: string): number {
    const ss1 = version1.split('.').map(Number), ss2 = version2.split('.').map(Number);
    const n = ss1.length, m = ss2.length;
    let i = 0, j = 0;
    while (i < n || j < m) {
        const a = i < n ? ss1[i] : 0;
        const b = j < m ? ss2[j] : 0;

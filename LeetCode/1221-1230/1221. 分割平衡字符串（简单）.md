...
![](https://pic.leetcode-cn.com/1630980197-vpDnIV-image.png)
...
![](https://pic.leetcode-cn.com/1630980647-QhYQNU-image.png)
...
public:
    int balancedStringSplit(string s) {
        ...
        for (int i = 0; i < n; ) {
            int j = i + 1;
            int score = (s[i] == 'L') ? 1 : -1;
            while (j < n && score != 0) score += (s[j++] == 'L') ? 1 : -1;
            i = j;
            ans++;
        }
        return ans;
...
class Solution:
    def balancedStringSplit(self, s: str) -> int:
        ...
        while i < n:
            j = i + 1
            score = 1 if s[i] == 'L' else -1
            while j < n and score != 0:
                score += 1 if s[j] == 'L' else -1
                j += 1
            i = j
            ans += 1
        return ans

...
public:
    vector<string> findRestaurant(vector<string>& list1, vector<string>& list2) {
        ...
        for (int i = 0; i < n; i++) indexMap[list1[i]] = i;
        vector<string> ans;
        int minv = 3000;
        for (int i = 0; i < m; i++) {
            string s = list2[i];
            if (indexMap.find(s) != indexMap.end()) {
                if (i + indexMap[s] < minv) {
                    minv = i + indexMap[s];
                    ans.clear();
                    ans.push_back(s);
                } else if (i + indexMap[s] == minv) {
                    ans.push_back(s);
                }
            }
        }
...
class Solution:
    def findRestaurant(self, list1: List[str], list2: List[str]) -> List[str]:
        index_map = {x: idx for idx, x in enumerate(list1)}
        minv = 3000
        ans = []        
        for i, x in enumerate(list2):
            if x in index_map:
                if i + index_map[x] < minv:
                    minv = i + index_map[x]
                    ans = [x]
                elif i + index_map[x] == minv:
                    ans.append(x)
        return ans
```
* 时间复杂度：$O(n + m)$
...
A0: 以 `Java` 为例，在 `ArrayList` 源码中的 `clear` 实现会为了消除容器对对象的强引用，遍历容器内的内容并置空来帮助 `GC`（垃圾回收）。

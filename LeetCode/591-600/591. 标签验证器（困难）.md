...
public:
    ...
    bool isValid(string s) {
        ...
        int n = s.length(), i = 0;
        while (i < n) {
            if (i + 8 < n && s.substr(i, 9) == CDATA1) {
                if (i == 0) return false;
                int j = i + 9;
                bool ok = false;
                while (j < n && !ok) {
                    if (j + 2 < n && s.substr(j, 3) == CDATA2) {
                        j += 3; ok = true;
                    } else {
                        j++;
                    }
                ...
                if (i == n - 1) return false;
                bool isEnd = (i + 1 < n && s[i + 1] == '/');
                int p = isEnd ? i + 2 : i + 1, j = p;
                while (j < n && s[j] != '>') {
                    if (!isupper(s[j])) return false;
                    j++;
                }
                if (j == n) return false;
                int len = j - p;
                if (len < 1 || len > 9) return false;
                string tag = s.substr(p, len);
                i = j + 1;
                if (!isEnd) {
                    d.push_back(tag);
                } else {
                    if (d.empty() || d.back() != tag) return false;
                    d.pop_back();
                    if (d.empty() && i < n) return false;
                }
            } else {
                if (i == 0) return false;
                i++;
...
class Solution:
    def isValid(self, s: str) -> bool:
        ...
        n, i = len(s), 0
        while i < n:
            if i + 8 < n and s[i:i+9] == CDATA1:
                if i == 0:
                    return False
                j = i + 9
                ok = False
                while j < n and not ok:
                    if j + 2 < n and s[j:j+3] == CDATA2:
                        j += 3
                        ok = True
                    else:
                        j += 1
                ...
                if i == n - 1:
                    return False
                isEnd = i + 1 < n and s[i + 1] == '/'
                p = i + 2 if isEnd else i + 1
                j = p
                while j < n and s[j] != '>':
                    if not s[j].isupper():
                        return False
...
function isValid(s: string): boolean {
    const CDATA1 = "<![CDATA[", CDATA2 = "]]>";
    let d = [];
    let n = s.length, i = 0;
    while (i < n) {
        if (i + 8 < n && s.substring(i, i + 9) === CDATA1) {
            if (i === 0) return false;
            let j = i + 9;
            let ok = false;
            while (j < n && !ok) {
                if (j + 2 < n && s.substring(j, j + 3) === CDATA2) {
                    j += 3; ok = true;
                } else {
                    j++;
                }
            ...
            if (i === n - 1) return false;
            let isEnd = s[i + 1] === '/';
            let p = isEnd ? i + 2 : i + 1, j = p;
            while (j < n && s[j] !== '>') {
                if (!s[j].match(/[A-Z]/)) return false;
                j++;
            }
            if (j === n) return false;
            let len = j - p;
            if (len < 1 || len > 9) return false;
            let tag = s.substring(p, j);
            i = j + 1;
            if (!isEnd) {
                d.push(tag);
            } else {
                if (d.length === 0 || d.pop() !== tag) return false;
                if (d.length === 0 && i < n) return false;
            }
        } else {
            if (i === 0) return false;
            i++;

...
public:
    ...
    bool check(const vector<int>& p, long k, int h) {
        if (k == 0) return false;
        long ans = 0;
        for (int x : p) ans += ceil(x * 1.0 / k);
        return ans <= h;
    }
};
```
...
class Solution:
    ...
    def check(self, p, k, h):
        return False if k == 0 else sum(math.ceil(x / k) for x in p) <= h
```
TypeScript 代码：
```TypeScript
function check(p: number[], k: number, h: number): boolean {
    if (k == 0) return false;
    let ans = 0;
    for (let x of p) ans += Math.ceil(x / k);
    return ans <= h;
}

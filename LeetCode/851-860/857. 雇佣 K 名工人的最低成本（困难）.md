...
class Solution {
public:
    double mincostToHireWorkers(vector<int>& qs, vector<int>& ws, int k) {
        int n = qs.size();
        vector<vector<double>> ds(n, vector<double>(2));
        for (int i = 0; i < n; i++) {
            ds[i][0] = static_cast<double>(ws[i]) / qs[i]; ds[i][1] = static_cast<double>(i);
        }
        sort(ds.begin(), ds.end(), [](const vector<double>& a, const vector<double>& b) {
            return a[0] < b[0];
        });
        priority_queue<int> q;
        double ans = 1e18;
        for (int i = 0, tot = 0; i < n; i++) {
            int cur = qs[static_cast<int>(ds[i][1])];
            tot += cur;
            q.push(cur);
            if (q.size() > k) {
                tot -= q.top();
            ...
            if (q.size() == k) ans = min(ans, tot * ds[i][0]);
        }
        return ans;
    }
};
...
```Python
class Solution:
    def mincostToHireWorkers(self, qs: List[int], ws: List[int], k: int) -> float:
        n = len(qs)
        ds = []
        for i in range(n):
            ds.append((ws[i] / qs[i], i))

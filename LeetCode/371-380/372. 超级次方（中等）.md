...
$$
a^{K} = a^{(\left \lfloor \frac{K}{10} \right \rfloor \times 10) + (K \bmod 10)} = a^{(\left \lfloor \frac{K}{10} \right \rfloor \times 10)} \times a^{(K \bmod 10)} = ({a^{\left \lfloor \frac{K}{10} \right \rfloor}) ^  {10}} \times a^{(K \bmod 10)}
$$
...
0. $a^K = 99^{2345}$
1. $99^{2345} = 99^{234 \times 10 + 5}$
2. $99^{234 \times 10 + 5} = 99^{234 \times 10} \times 99^{5}$
3. $99^{234 \times 10} \times 99^{5} = {(99^{234})}^{10} \times 99^{5}$
...
...
public:
    ...
    int dfs(int a, vector<int>& b, int u) {
        if (u == -1) return 1;
        return (powmod(dfs(a, b, u - 1), 10) * powmod(a, b[u])) % MOD;
    }
...
class Solution:
    ...
    def dfs(self, a, b, u):
        if u == -1:
            return 1
        return (self.powmod(self.dfs(a, b, u - 1), 10) * self.powmod(a, b[u])) % self.MOD
    ...
    def powmod(self, a, b):
        ...
        return ans
```
* 时间复杂度：假设 $b$ 数组所代表的数字为 $K$，使用快速幂的复杂度为 $O(\log{K})$，或者说是 $O(n\log{10})$，其中 $n$ 为数组 $b$ 的长度，数字 $10$ 所代表的含义是计算一个次方为 $10$ 以内的值；而不使用快速幂的复杂度为 $O(n \times 10)$
